...
state("Ace7Game")
{
    ...
    int score: 0x03A5B780, 0x55C; // 0 when transition

    // Ingametime according to the game (perfectly mirrors the mission result screen)
    float IGT: 0x03A5B780, 0x528; // undefined when transition

    // Number of kills (might be useful?)
    int kills: 0x03A5B780, 0x520, 0x88, 0x5A0; // 0 when transition

    // Some sort of state?
    int state: 0x03A5B780, 0x450, 0x428, 0x5A4; // undefined when transition
    ...
    // Mission number
    int missionID: 0x03A5B780, 0x470; // persists into replay

    // Pause flag
    int paused: 0x03A5B780, 0x3E0; // Game pause flag (apparently different depending on the mission ... see list below)
